# ISLES 2022 2.5D public snapshot — `isles2022-2p5d-v1.0-interview`

## Purpose
A stable portfolio snapshot for hiring review and external technical inspection.

## Snapshot type
Stable review build.

## Reproduced scope
- ISLES 2022 lesion segmentation with 2.5D ConvNeXt pipeline
- A 5-slice model (Dice-OHEM-BCE, 100ep) and a 7-slice dilated model (Tversky-OHEM-BCE, EMA, 150ep)
- Probability-map average ensemble of the two models
- Single-model and ensemble evaluation with TTA support

## Key results in this snapshot
- val mean Dice: **0.722** (ensemble), 0.704 (5-slice model), 0.690 (7-slice dilated model)
- test mean Dice: **0.631** (ensemble), ~0.58 (5-slice model), 0.579 (7-slice dilated model)
- vs. 3D U-Net baseline (test 0.514): **+0.117 (+22.8% relative)**

## Main strengths in this snapshot
- Lightweight 2.5D design: 2D ConvNeXt encoder + multi-slice channel stacking
- Tversky loss (β=0.7) for improved small-lesion recall
- EMA for stable checkpointing
- Ensemble diversity from different slice spacings and loss functions

## Known limitations
- Protected medical data is not bundled
- GitHub Release text needs to be published in the web UI
- Smoke test validates repository wiring, not full model quality
