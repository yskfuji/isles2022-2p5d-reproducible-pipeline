...
def _extract_metric(row: dict[str, Any], best_thr: float, key: str) -> Any:
    specific = row.get(f"{key}@{best_thr:g}")
...
def main() -> None:
    ...
    rows = _read_json(eval_dir / "metrics.json")
    ...
    for r in rows:
        ...
        if not cid:
            continue
        dice_best = float(_extract_metric(r, best_thr, "dice") or 0.0)
        tp = _safe_int(_extract_metric(r, best_thr, "tp_vox"))
        fp = _safe_int(_extract_metric(r, best_thr, "fp_vox"))
        fn = _safe_int(_extract_metric(r, best_thr, "fn_vox"))
        precision = float(tp) / float(tp + fp) if (tp + fp) > 0 else 0.0
        recall = float(tp) / float(tp + fn) if (tp + fn) > 0 else 0.0
        detected = _extract_metric(r, best_thr, "detected")
        cases.append(
            {
                "case_id": str(cid),
                "dice_best": dice_best,
                ...
                "pred_vox": _safe_float(_extract_metric(r, best_thr, "pred_vox")),
                "tp_vox": float(tp),
                "fp_vox": float(fp),
                "fn_vox": float(fn),
                "precision": precision,
                "recall": recall,
                "error_tag": _classify_case(detected=detected, precision=precision, recall=recall),
                "fp_cc": _safe_float(_extract_metric(r, best_thr, "fp_cc")),
    ...
    report = {
        ...
        "worst_cases": [
            {
                "case_id": c["case_id"],
                "dice_best": c["dice_best"],
                "gt_vox": c["gt_vox"],
                "pred_vox": c["pred_vox"],
                "fp_vox": c["fp_vox"],
                "fn_vox": c["fn_vox"],
                "gt_size_bucket": c["gt_size_bucket"],
            }
            for c in worst
        ],
    }
